...
select a.firsts,b.seconds,c.count,c.sum_delta,a.poker_session_date
from
(select count(*) firsts,poker_session_date
  from poker_sessions
  where buy_in = 45000 and place = 1
  group by poker_session_date) a
left outer join
(select count(*) seconds,poker_session_date
  from poker_sessions
  where buy_in = 45000 and place = 2
...
where a.firsts = 1 and b.seconds = 1
